...
export default function Q5() {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      const status = await checkStatus("6")
  ...
  function cekJawaban() {
    ...
    if (jawaban === userJawab) {
      ...
      if (percobaan === 0) {
        setStatus(false)
        setTimer(60)
        sessionStorage.setItem("status6", "false")
      }
  ...
  const pilihan = [
    { id: "A", text: "SELECT No, Jurusan FROM MAHASISWA" },
    { id: "B", text: "SELECT No and Jurusan FROM MAHASISWA" },
    { id: "C", text: "SELECT No, Nama FROM MAHASISWA" },
    { id: "D", text: "SELECT Nama, Jurusan FROM MAHASISWA" },
    { id: "E", text: "SELECT Jurusan FROM MAHASISWA" },
  ]

  useEffect(() => {
    setStatus(!sessionStorage.getItem("status6"))
  }, [])
  ...
  useEffect(() => {
    ...
    if (timer === 0) {
      setStatus(true);
      setPercobaan(2);
      sessionStorage.removeItem("status6");
    }
  ...
  return (
    <>
      ...
      <div className="w-[90%] md:w-[50%] mx-auto mt-5 flex items-center justify-between text-white">
        <h1 className="text-2xl font-bold">SOAL 6</h1>
        <Link href="/quiz">
      ...
      {togglePenjelasan && (<div className="mb-5 text-white mx-auto mt-5 w-[90%] md:w-[50%] rounded-lg p-5 space-y-4 bg-gray-900/50">
        ...
        <p>Seperti yang kita tahu, kalo pengen ambil kolom yang spesifik, kita tinggal panggil nama kolomnya</p>
        <p>Nah, Jinggy pengen ambil <b>No</b> sama <b>Jurusan</b> nih</p>
        <p>Tinggal tambahin aja nama kolomnya</p>
        <p className="text-red-400">SELECT <span className="text-yellow-400">No, Jurusan</span> FROM MAHASISWA</p>
        <p>Jadi tinggal dipisah pake koma "," begituuu kalo lebih dari 1 kolom.</p>
      </div>)}

      {/* SOAL */}
      <div className="text-white mt-5 mx-auto w-[90%] md:w-[50%]">
        <p className="font-bold mb-2 text-green-400">INSTRUKSI</p>
        <p>Jiggy ingin mengambil data berupa "No" dan "Jurusan" pada tabel MAHASISWA, bantu Jiggy untuk membuat query sql untuk mendapatkan data yang diinginkan!</p>
      </div>
